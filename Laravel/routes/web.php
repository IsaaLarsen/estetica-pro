...
// Tela de alteração de senha (incluindo primeiro acesso)
Route::get('/me/senha', [LoginController::class, 'formPassword'])
    ->name('me.senha.form')
    ->middleware(['panel']);
...
// ===========================
// Funcionários CRUD (sem show) + Ações extras
// ===========================
Route::resource('funcionarios', FuncionarioController::class)
    ->except(['show'])
    ->middleware(['panel', 'role:admin']);

// 🔐 Redefinição de senha do usuário vinculado ao funcionário (por ID)
Route::post('/funcionarios/{funcionario}/reset-senha', [FuncionarioController::class, 'resetSenha'])
    ->name('funcionarios.resetSenha')
    ->middleware(['panel', 'role:admin']);

// 🔐 Ações rápidas por CPF (menu botão direito)
Route::post('/funcionarios/reset-senha-por-cpf', [FuncionarioController::class, 'resetSenhaPorCpf'])
    ->name('funcionarios.reset-senha-por-cpf')
    ->middleware(['panel', 'role:admin']);

Route::post('/funcionarios/alterar-tipo-por-cpf', [FuncionarioController::class, 'alterarTipoPorCpf'])
    ->name('funcionarios.alterar-tipo-por-cpf')
    ->middleware(['panel', 'role:admin']);

// 🔐 Desbloquear conta (menu botão direito)
Route::post('/funcionarios/desbloquear-conta', [FuncionarioController::class, 'desbloquearConta'])
    ->name('funcionarios.desbloquear-conta')
    ->middleware(['panel', 'role:admin']);

// 🔐 Alias adicional para compatibilidade com JS antigo
Route::post('/funcionarios/desbloquear-por-cpf', [FuncionarioController::class, 'desbloquearConta'])
    ->name('funcionarios.desbloquear-por-cpf')
    ->middleware(['panel', 'role:admin']);

// ===========================
// Serviços CRUD (sem show) - APENAS ADMIN
// ===========================
...
// ===========================
// Clientes CRUD (sem show) - APENAS ADMIN
// ===========================
...
// ===========================
// Cargos CRUD (sem show) - APENAS ADMIN
// ===========================
...
// ROTA PARA MINHA AGENDA (FUNCIONÁRIOS)
...
// ROTA PARA AGENDA COMPLETA (ADMIN)
...
Route::prefix('relatorios')
    ...
    ->group(function () {
        ...
        Route::get('/faturamento', [RelatorioController::class, 'faturamento'])
            ->name('relatorios.faturamento');

        Route::get('/faturamento/pdf', [RelatorioController::class, 'faturamentoPdf'])
        ...
        Route::get('/servicos/pdf', [RelatorioController::class, 'servicosPdf'])
            ->name('relatorios.servicos.pdf');
    });

// ===========================
// Feedbacks (APENAS ADMIN)
// ===========================
Route::middleware(['panel', 'role:admin'])->group(function () {
