...
@section('content')
    ...
    <div class="page-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            ...
            {{-- Botão "Novo Cliente" só para ADMIN (funcionário não tem acesso à rota clientes.create) --}}
            @if(!$isFuncionario)
    ...
    {{-- KPIs --}}
    <div class="stats-grid" style="display:grid; grid-template-columns: repeat({{ $isFuncionario ? 3 : 4 }}, minmax(0,1fr)); gap:16px; margin-bottom:24px;">
        @php
            // KPIs diferentes para funcionário vs admin
            if ($isFuncionario) {
                $kpis = [
    ...
    <div class="filters-bar" style="display:flex; gap:12px; align-items:center; margin-bottom:18px;">
        <form method="GET" action="{{ route('dashboard') }}" style="display:flex; gap:12px; flex-wrap:wrap;">
            ...
            {{-- Combo de funcionários só aparece para ADMIN.
                 Funcionário SEMPRE está filtrado nele mesmo pelo controller. --}}
            @if(!$isFuncionario)
