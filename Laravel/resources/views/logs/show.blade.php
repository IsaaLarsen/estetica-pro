...
@php
    ...
    $formatarPorCampo = function ($campo, $v) use ($formatarValorBase, $funcionarios, $clientes, $servicos) {
        ...
        // Para timestamps ISO (tipo 2025-11-24T19:29:56.000000Z),
        // vamos tentar SEMPRE formatar bonito quando for created_at/updated_at
        if (in_array($campo, ['created_at', 'updated_at'], true)) {
            try {
                ...
                // se der erro, volta formato base
...
<div style="background:white; padding:24px; border-radius:16px; box-shadow:0 4px 20px rgba(0,0,0,0.08);">
    ...
    <div style="display:grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap:12px; margin-bottom:24px; font-size:14px;">
        ...
        <div>
            ...
            <div><strong>Data/Hora do log:</strong> {{ $log->created_at->timezone(config('app.timezone','America/Sao_Paulo'))->format('d/m/Y H:i:s') }}</div>
        </div>
        <div>
