...
<div style="background:white; padding:24px; border-radius:16px; box-shadow:0 4px 20px rgba(0,0,0,0.08);">
    ...
    <form method="GET" action="{{ route('logs.index') }}"
          style="margin-bottom:16px; display:grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap:8px;">
        <input type="text" name="usuario_nome" placeholder="Usuário"
               value="{{ request('usuario_nome') }}"
        ...
        <div style="grid-column:1 / -1; display:flex; justify-content:flex-end; gap:8px; margin-top:4px;">
            <a href="{{ route('logs.index') }}"
               style="padding:8px 16px; border-radius:8px; background:#f3f4f6; text-decoration:none; color:#374151; font-size:14px;">
               Limpar
            </a>
            <button type="submit"
                    style="padding:8px 16px; border-radius:8px; background:linear-gradient(135deg, var(--primary), var(--secondary)); color:white; border:none; font-size:14px; cursor:pointer;">
                Filtrar
            </button>
        </div>
    </form>
